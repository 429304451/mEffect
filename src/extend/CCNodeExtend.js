	...
	if (fileName === undefined) {
		...
		if (fileName[0] === "#") {
			...
			if (spriteFrame) {
				...
				console.log("%s does not exit", fileName);
			}
		} else {
...
cc.Node.prototype.bindTouch = function (args) {
	...
	if (args.onTouchBegan != undefined) {
		...
		// Changwei 2017/7/18 加这个主要是方便TV手指调用
...
cc.Node.prototype.bindTouchLocate = function () {
    ...
    this.bindTouch({
        ...
        onTouchEnded: function (touch, event) {
            ...
            let pw = winSize.width, ph = winSize.height;
            if (self.getParent() !== null) {
                let size = self.getParent().getContentSize();
                pw = size.width;
                ph = size.height;
            }
            console.log("Node Location: ", self.getPositionX(), self.getPositionY(), "Percentage:", self.getPositionX()/pw, self.getPositionY()/ph);
        },
    });
    return this;
};
...
// 快速绑定点击函数
cc.Node.prototype.bt = function (fn) {
